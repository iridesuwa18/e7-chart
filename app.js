...
function renderChart() {
  ...
  // Compute positions (spread or raw)
  let posMap = {};
  ...
  heroes.forEach(h => {
    ...
    dot.style.top    = pos.y + "%";
    dot.style.zIndex = isHighlighted ? "30" : "";

    const meta = RARITY_META[h.rarity] || RARITY_META["5r"];
    const inner = document.createElement("div");
    inner.className = "hero-dot-inner";
    // Green border if highlighted, else rarity border
    if (isHighlighted) {
    ...
    // Stack badge — shown when spread is off and heroes overlap
    if (!spreadEnabled && stackCounts[h.id] && stackCounts[h.id] > 1) {
      const badge = document.createElement("div");
      badge.className = "hero-dot-stack-badge";
      badge.textContent = "×" + stackCounts[h.id];
    ...
    dot.addEventListener("pointerdown", e => {
      ...
      const dotCenterX = (rawX / 100) * rect.width;
      const dotCenterY = (rawY / 100) * rect.height;
    ...
    // Store for ordered append — highlighted goes last (painted on top)
    dot._isHighlighted = isHighlighted;
    chartDots.push(dot);
  });

  // Non-highlighted first, highlighted last → guaranteed on top regardless of z-index
  chartDots.filter(d => !d._isHighlighted).forEach(d => chart.appendChild(d));
  chartDots.filter(d =>  d._isHighlighted).forEach(d => chart.appendChild(d));
}
