...
function renderChart() {
  ...
  heroes.forEach(h => {
    ...
    dot.addEventListener("pointerdown", e => {
      ...
      const hxPx = (x / 100) * rect.width;
      const hyPx = (y / 100) * rect.height;
  ...
  list.forEach(h => {
    ...
    card.innerHTML = `
      ...
      <div class="hero-card-actions">
        <button class="icon-btn btn-lock" title="Lock">${lockIcon}</button>
        <button class="icon-btn btn-edit" title="Edit">✏️</button>
        <button class="icon-btn btn-del"  title="Delete">🗑️</button>
      </div>`;

    card.querySelector(".btn-lock").addEventListener("click", e => { e.stopPropagation(); toggleLock(h.id); });
...
function cropAndSave() {
  ...
  ctx.clip();
  // Draw image at its current pan/zoom, scaled to 48px editor size
  const scale = 48 / EDITOR_SIZE;
  ctx.drawImage(
    editorImg,
    editorPanX * scale,
    editorPanY * scale,
...
function onModalConfirm() {
  ...
  if (editingId !== null) {
    heroes = heroes.map(h => h.id === editingId
      ? { ...h, name, rarity, role, element, vType, vScore, hType, hScore, notes, iconData }
      : h
    );
  } else {
    heroes.push({
