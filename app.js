...
// Display strings only — renamed per Rebuild Spec Section 6.2. The
// underlying categorization (qdQuadrantOf, TL/TR/BL/BR keying off
// vType/hType) is untouched; SPD→Selfish, TNK→Selfless, SUR→Reaction,
// SST→Engage is a label swap, not a data-model change (that's Section 1
// legacy-field territory, out of scope here).
const QD_QUADRANT_LABEL = {
  TL: "Reaction + Selfish",
  TR: "Engage + Selfish",
  BL: "Reaction + Selfless",
  BR: "Engage + Selfless",
};
...
/* ═══════════════════════════════════════
   ...
   qdSuggestForSlot (above) is the legacy quadrant/tier/chain engine
   (Section 8.3) — kept fully intact and still reachable, just no longer
   the primary driver. This dispatcher is the single point every real
   pick source (Suggest, Autofill, Rep. Curr.) now goes through: once
   any Factor is ticked, Factor-based ranking takes over; with nothing
   ticked, it falls straight back to the old system, unchanged. The
   tutorial "how Quick Draft picks a team" mould (qdSimulateChain) is a
   separate, hardcoded-example path and intentionally does NOT go
   through this dispatcher — it always demonstrates the pure quadrant
   system regardless of what's ticked in a real, in-progress draft.
═══════════════════════════════════════ */
function qdSuggestForNextIdx(nextIdx, draftArr, banProtectEl) {
  if (qdTickedFactorIds.size > 0) {
    const factorScored = qdFactorSuggestForSlot(nextIdx, draftArr);
    // Graceful fallback, same philosophy as qdSuggestForSlot's own pool
    // fallbacks: if nobody in the roster currently matches ANY ticked
    // Factor, don't show an empty list — fall back to the quadrant/
    // chain ranking rather than leaving Suggest/Autofill with nothing.
    if (factorScored.length) return factorScored;
  }
  return qdSuggestForSlot(nextIdx, draftArr, banProtectEl);
}
...
/* Simulates a full 5-slot Quick Draft from a single fixed first pick,
   ...
   remaining slot takes whatever qdSuggestForSlot ranks #1 — Section 9.3
   applies the same 3-Selfless/2-Selfish default (via
   qdRequiredSideForDraft) that Autofill uses, so example teams stay
   consistent with what pressing Autofill would actually produce; the
   pure-quadrant ranking source (qdSuggestForSlot, not the Factor
   dispatcher — see its own comment above) is otherwise unchanged. Never
   touches the global quickDraft array or qdBanProtectElement. */
...
async function onModalConfirm() {
  ...
  // Alt / ghost stats. vType/vScore/hType/hScore stay exactly as before —
  // Quick Draft's ghost-build ranking still reads those. selfishScore/
  // selflessScore/reactions/engagements are new (Section 6 open-decision:
  // "extend ghost mode to the new axes") and drive the ghost dot's chart
  // position instead.
  ...
  if (editingId !== null) {
    heroes = heroes.map(h => h.id === editingId
      ? { ...h, name, rarity, role, element, vType, vScore, hType, hScore, notes, iconData, locked, pvpTag, altStats, selfishScore, selflessScore, reactions, engagements }
      : h
    );
  } else {
    heroes.push({
