...
(function init() {
  ...
  document.getElementById("btn-github-heroes").addEventListener("click", () => {
    openFileManager(null, "heroes", (url) => {
      fIconData.value = url;
      updateIconPreview(url);
    });
  });
  document.getElementById("btn-github-icons").addEventListener("click", () => {
    openFileManager(null, "icons", (url) => {
      fIconData.value = url;
      updateIconPreview(url);
    });
  });
...
function openFileManager(croppedData, folder, onSave) {
      ...
      <div id="fm-box" style="background:#071220;border:1px solid #1a3050;border-top:2px solid #c9a227;border-radius:6px;padding:22px 22px 18px;width:440px;max-width:96vw;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.8)">
        ...
        <div id="fm-step-mgr" style="display:none">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid #6a5010">
            <span style="font-family:Cinzel,serif;color:#e8c84a;font-size:13px;letter-spacing:.25em" id="fm-mgr-title">📁 REPO FILE MANAGER</span>
            <button id="fm-mgr-cancel" style="background:none;border:none;color:#607a90;font-size:18px;cursor:pointer;line-height:1">x</button>
          </div>

          <!-- Upload mode: new image + filename row -->
          <div id="fm-upload-row" style="background:#0b1a2e;border:1px solid #1a3050;border-radius:4px;padding:12px 14px;margin-bottom:18px;display:flex;gap:14px;align-items:flex-start;flex-wrap:wrap">
          ...
          <!-- Picker mode: selected image preview -->
          <div id="fm-picker-selected" style="display:none;background:#0b1a2e;border:1px solid #1a3050;border-radius:4px;padding:10px 14px;margin-bottom:14px;flex-direction:row;align-items:center;gap:12px">
            <img id="fm-picker-thumb" src="" alt="" style="width:48px;height:48px;border-radius:4px;object-fit:cover;border:2px solid #c9a227;flex-shrink:0"/>
            <div>
              <div style="font-family:Cinzel,serif;font-size:9px;color:#607a90;letter-spacing:.2em;margin-bottom:3px">SELECTED</div>
              <div id="fm-picker-name" style="font-size:12px;color:#c9a227;font-family:'Crimson Pro',serif;word-break:break-all"></div>
            </div>
          </div>
          ...
          <div id="fm-existing-section">
            <div style="font-family:Cinzel,serif;font-size:9px;color:#607a90;letter-spacing:.2em;margin-bottom:8px;display:flex;align-items:center;gap:8px">
              <span id="fm-grid-label">EXISTING IMAGES</span>
          ...
          <div style="display:flex;gap:8px;justify-content:flex-end;padding-top:14px;border-top:1px solid #1a3050">
            <button id="fm-discard" style="background:#0b1a2e;border:1px solid #1a3050;color:#607a90;padding:5px 12px;border-radius:3px;font-size:11px;font-family:Cinzel,serif;letter-spacing:.1em;cursor:pointer">Cancel</button>
  ...
  let replaceTarget  = null; // {name,sha,url} | null — upload mode: replace file; picker mode: unused
  let pendingDeletes = new Set();
  let pickerSelected = null; // {name,url} | null — picker mode only

  const isPickerMode = !croppedData;
  ...
  function setHint() {
    if (isPickerMode) return; // no hint needed in picker mode
  ...
  function renderGrid() {
    ...
    existingImages.forEach(img => {
      const isReplace  = !isPickerMode && replaceTarget?.name === img.name;
      const isDelete   = !isPickerMode && pendingDeletes.has(img.name);
      const isPicked   =  isPickerMode && pickerSelected?.name === img.name;
      const cell = document.createElement("div");
      cell.style.cssText = `position:relative;border:2px solid ${(isReplace||isPicked)?"#c9a227":isDelete?"#c0392b":"#1a3050"};border-radius:4px;overflow:hidden;background:#0b1a2e;cursor:${isDelete?"default":"pointer"};opacity:${isDelete?0.35:1};transition:all 0.15s`;
      cell.innerHTML = `
        <img src="${img.url}" alt="${img.name}" style="width:100%;aspect-ratio:1;object-fit:cover;display:block"/>
        ${(isReplace||isPicked) ? `<div style="position:absolute;inset:0;background:rgba(201,162,39,.25);display:flex;align-items:center;justify-content:center"><span style="font-family:Cinzel,serif;font-size:8px;color:#c9a227;background:rgba(0,0,0,.85);padding:2px 6px;border-radius:2px;letter-spacing:1px">${isPicked?"SELECTED":"REPLACE"}</span></div>` : ""}
        <div style="padding:3px 5px;display:flex;align-items:center;gap:3px;background:rgba(0,0,0,.75)">
          ...
          ${!isPickerMode ? `<button data-del="${img.name}" style="background:none;border:none;color:${isDelete?"#e87070":"#607a90"};font-size:13px;cursor:pointer;padding:0 1px;line-height:1;flex-shrink:0;font-weight:700">${isDelete?"↩":"×"}</button>` : ""}
        </div>`;
      // Click image
      cell.querySelector("img").addEventListener("click", () => {
        if (isPickerMode) {
          // Picker mode: clicking selects the image
          pickerSelected = pickerSelected?.name === img.name ? null : img;
          updatePickerSelectedUI();
          renderGrid();
          document.getElementById("fm-save").disabled = !pickerSelected;
          return;
        }
        // Upload mode: clicking picks as replace target
        if (isDelete) return;
      ...
      // Delete button (upload mode only)
      const delBtn = cell.querySelector("button[data-del]");
      if (delBtn) {
  ...
  function updatePickerSelectedUI() {
    const sel = document.getElementById("fm-picker-selected");
    if (pickerSelected) {
      sel.style.display = "flex";
      document.getElementById("fm-picker-thumb").src = pickerSelected.url;
      document.getElementById("fm-picker-name").textContent = pickerSelected.name;
    } else {
  ...
  // --- populate and show ---
  const isPicker = isPickerMode; // alias for clarity in closures
  document.getElementById("fm-folder-label").textContent  = `assets/${folder}/`;
  document.getElementById("fm-ext-label").textContent     = `.${ext}`;
  document.getElementById("fm-upload-row").style.display  = isPicker ? "none" : "flex";
  document.getElementById("fm-picker-selected").style.display = "none";
  if (!isPicker) {
    document.getElementById("fm-preview-img").src = croppedData;
    document.getElementById("fm-filename").value  = "";
    document.getElementById("fm-save-hint").textContent = "Enter a filename above";
  }
  document.getElementById("fm-mgr-title").textContent    = isPicker ? "📁 CHOOSE FROM GITHUB" : "📁 REPO FILE MANAGER";
  document.getElementById("fm-grid-label").textContent   = isPicker ? "CLICK AN IMAGE TO USE IT" : "EXISTING IMAGES";
  document.getElementById("fm-save").textContent         = isPicker ? "Use Selected" : "Save";
  document.getElementById("fm-save").disabled            = isPicker; // disabled until something selected
  document.getElementById("fm-discard").textContent      = "Cancel";
  ...
  async function doVerify() {
    ...
    try {
      ...
      pickerSelected = null;
      renderGrid();
      if (!isPickerMode) { updateReplaceUI(); setHint(); }
      else { updatePickerSelectedUI(); }
  ...
  rebind("fm-save", async () => {
    ...
    // ── Picker mode: just return the selected URL ──
    if (isPickerMode) {
      if (!pickerSelected) { showErr("fm-save-err", "Click an image to select it first."); return; }
      const cb = _fmCallback;
      closeFileManager();
      if (cb) cb(pickerSelected.url);
      return;
    }

    // ── Upload mode: upload cropped image, optionally delete others ──
    let finalFilename;
    if (replaceTarget) {
    ...
    try {
      ...
      const cb = _fmCallback; // save before closeFileManager nulls it
      closeFileManager();
      if (cb) cb(upJson.url);
    } catch (e) {
