...
function ImagePicker({ value, onChange, allImages=[] }) {
  ...
  return (
    <div>
      ...
      {cropSrc && <ImageCropModal src={cropSrc} onSave={v=>{onChange(v);setCropSrc(null);}} onClose={()=>setCropSrc(null)}/>}
    </div>
  );
}
  ...
  return (
    <Modal title={`DRAFT EDITOR — ${f.name||"Unnamed Hero"}`} onClose={onClose} width={700} maxH="93vh">
      ...
      <Field label="Icon Image">
        <ImagePicker value={f.iconData||""} onChange={v=>setF(x=>({...x,iconData:v}))} allImages={allImages}/>
      </Field>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
      ...
      <div style={{ padding:"10px 16px", borderBottom:`1px solid ${T.border}`, display:"flex", gap:8, alignItems:"center", flexShrink:0, flexWrap:"wrap" }}>
        ...
        <span style={{ marginLeft:"auto", fontSize:11, color:T.dim, fontFamily:"'Crimson Pro',serif", fontStyle:"italic" }}>Click ✏️ to open Draft editor. Add heroes via ＋ Hero on the Chart.</span>
      </div>
          ...
          return (
            <div key={h.id} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:5, padding:"10px 11px", display:"flex", flexDirection:"column" }}>
              ...
              <div style={{ marginTop:"auto", paddingTop:8, borderTop:`1px solid ${T.border}` }}>
                <Btn onClick={()=>setEdit({...h})} style={{ width:"100%", textAlign:"center" }}>✏️ Draft Editor</Btn>
              </div>
            </div>
          );
        })}
...
function Slot({ idx, hero, team, isActiveTeam, active, setActive, onRemove, highlight, settings }) {
  ...
  return (
    <div onClick={()=>setActive({team,idx})} className={gc}
      style={{ aspectRatio:"1", maxHeight:"10vh", width:"100%", background:hero?T.card:T.bg, border:`1px solid ${hero?T.border:T.dim}`, borderRadius:4, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", cursor:"pointer", position:"relative", transition:"box-shadow 0.15s,border-color 0.15s", overflow:"hidden" }}>
...
function TeamPanel({ label, team, teamKey, opp, active, setActive, onRemove, draftData }) {
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", padding:"8px 10px", gap:6 }}>
      ...
      {/* 3+2 slots — capped so they never overflow into the analysis area */}
      <div style={{ flexShrink:0, maxHeight:"28vh" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:4, marginBottom:5 }}>
          <div style={{gridColumn:"2 / span 2"}}><Slot idx={3} hero={team[3]} team={teamKey} isActiveTeam={isActiveTeam} active={active} setActive={setActive} onRemove={onRemove} highlight={team[3]?highlightOf(team[3]):null} settings={draftData.settings}/></div>
          <div style={{gridColumn:"4 / span 2"}}><Slot idx={4} hero={team[4]} team={teamKey} isActiveTeam={isActiveTeam} active={active} setActive={setActive} onRemove={onRemove} highlight={team[4]?highlightOf(team[4]):null} settings={draftData.settings}/></div>
        </div>
        {/* Name list */}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
      ...
      <div style={{ background:T.panel, borderTop:`1px solid ${T.border}`, flexShrink:0 }}>
        {/* Top row: active slot label + search + sort */}
        ...
        <div style={{ padding:"4px 12px 0", display:"flex", gap:4, alignItems:"center", overflowX:"auto" }}>
          ...
          {["All",...Object.keys(EL_META)].map(el => (
            <button key={el} onClick={()=>setFEl(el)}
              style={{ background:fEl===el?(EL_META[el]?.color||T.gold):T.card, border:`1px solid ${fEl===el?(EL_META[el]?.color||T.gold):T.border}`, color:"#fff", padding:"2px 8px", borderRadius:2, fontSize:10, fontFamily:"Cinzel,serif", flexShrink:0, cursor:"pointer" }}>
              {el==="All" ? "All" : <span style={{ display:"flex", alignItems:"center", gap:3 }}><Ico src={draftData.settings?.elementIcons?.[el]||""} size={12} fallback={elIcon(el,draftData.settings)}/>{EL_META[el]?.label}</span>}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding:"4px 12px 0", display:"flex", gap:4, alignItems:"center", overflowX:"auto" }}>
          ...
          {["All",...Object.keys(CL_META)].map(cl => (
            <button key={cl} onClick={()=>setFCl(cl)}
              style={{ background:fCl===cl?T.gold:T.card, border:`1px solid ${fCl===cl?T.gold:T.border}`, color:fCl===cl?"#04090f":T.dim, padding:"2px 8px", borderRadius:2, fontSize:10, fontFamily:"Cinzel,serif", flexShrink:0, cursor:"pointer" }}>
              {cl==="All" ? "All" : <span style={{ display:"flex", alignItems:"center", gap:3 }}><Ico src={draftData.settings?.classIcons?.[cl]||""} size={12} fallback={clsIcon(cl,draftData.settings)}/>{CL_META[cl]?.label}</span>}
            </button>
          ))}
        </div>

        {/* Unique role filter — only shown if any exist */}
        ...
        <div style={{ display:"flex", gap:5, padding:"5px 12px 7px", overflowX:"auto", overflowY:"hidden", height:90 }}>
          ...
          {roster.length===0 && <div style={{ color:T.dim, fontSize:12, fontStyle:"italic", alignSelf:"center", fontFamily:"'Crimson Pro',serif", padding:"0 8px" }}>All heroes placed or no heroes match filter.</div>}
        </div>
      </div>
    </div>
  );
